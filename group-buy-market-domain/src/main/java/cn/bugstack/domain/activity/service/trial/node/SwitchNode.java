...
public class SwitchNode extends AbstractGroupBuyMarketSupport {
    ...
    protected TrialBalanceEntity doApply(MarketProductEntity requestParameter, DefaultActivityStrategyFactory.DynamicContext dynamicContext) throws Exception {
        ...
        //判断是否降级
        if(activityRepository.downgradeSwitch()){//也就是downgradeSwitch为1 拦截
            log.info("拼团活动降级拦截 {}", userId);
            throw new AppException(ResponseCode.E0003.getCode(), ResponseCode.E0003.getInfo());
        }

        //判断切面范围
        if(!activityRepository.cutRange(userId)){//也就是userId<cutRange 的非 拦截
